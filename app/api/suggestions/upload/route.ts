...
const POST_HANDLER = async (request: NextRequest) => {
  ...
  try {
    ...
    if (session?.discordId) owner = session.discordId
  } catch {
    // No / invalid session — fine, suggestions are open to anyone.
  }
  ...
  const key = `suggestions/${owner}/${timestamp}-${rand}-${safeBase}.${ext}`
