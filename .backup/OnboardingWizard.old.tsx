...
export default function OnboardingWizard() {
  ...
  const [s, setS] = useState<WizardState>(() => {
    ...
    // This ensures wizard data (mafiaName, city, etc.) is available before any effects run
    let saved: Partial<WizardState> = {};
    let shouldShowSubmitting = false;
    ...
    try {
      if (typeof window !== "undefined") {
        ...
        if (raw) {
          ...
          saved = {
            ...parsed,
            step: parsed?.step === 5 ? 4 : (parsed?.step ?? 0),
            turtles: migratedTurtles,
            seenNames: Array.isArray(parsed?.seenNames) ? parsed.seenNames : [],
          };

          // Check if we're returning from OAuth with a pending claim
          // If so, show submitting state immediately to avoid flash of step 4
          const url = new URL(window.location.href);
          const hasDiscordCallback = url.searchParams.has("discordId");
          const hasPendingClaim = localStorage.getItem(PENDING_CLAIM_KEY) === "1";
          const hasWizardData = !!(parsed?.memberId && parsed?.mafiaName);

          if (hasDiscordCallback && hasPendingClaim && hasWizardData) {
            shouldShowSubmitting = true;
          }
        }
      }
    } catch { }
    ...
    return {
      ...
      ...saved, // Merge saved state on top of defaults
      // Override submitting if we're returning from OAuth with pending claim (must come after ...saved)
      submitting: shouldShowSubmitting || saved.submitting || false,
    };
  });
  ...
  // Track if we've already processed URL params to prevent re-running after URL is cleared
  const hasProcessedParams = useRef(false);

  // ✅ Pick up Discord callback params (/?discordId=...&discordJoined=1&sessionId=...)
  useEffect(() => {
    // Prevent re-running after URL params are cleared (e.g., after edit flow starts)
    ...
    try {
      ...
      // Mark as processed if we have any special URL params
      if (discordId || isEdit) {
        hasProcessedParams.current = true;
      }

      // ✅ Check if user is already logged in (no URL params = fresh page load)
      if (!discordId && !isEdit) {
        hasProcessedParams.current = true; // Mark as processed to prevent re-runs
        (async () => {
          try {
            ...
            if (meRes.ok) {
              ...
              if (meData.discordId) {
                ...
                if (lookupRes.ok) {
                  ...
                  if (lookupData.found && lookupData.memberId) {
                    // User has a profile, redirect to dashboard
                    router.push(`/dashboard/${lookupData.memberId}`);
                    return;
                  }
                }
              }
            ...
            // Session check failed, continue to show login screen
          }
      ...
      if (discordId) {
        setS((p) => {
          // If discordId changed, clear memberId to prevent cross-account issues
          const shouldClearMember = p.discordId && p.discordId !== discordId;
          return {
            ...p,
            discordId,
            ...
            // Clear memberId if this is a different Discord account
            memberId: shouldClearMember ? "" : p.memberId,
          };
        });
        ...
        (async () => {
          ...
          try {
            ...
            // Not found -> Check if user has pending claim (just completed wizard)
            clearTimeout(timeoutId);
            let hasPendingClaim = false;
            try {
              hasPendingClaim = localStorage.getItem(PENDING_CLAIM_KEY) === "1";
            } catch { }

            // Check localStorage directly for wizard data (state may not be updated yet)
            let hasWizardData = false;
            try {
              const raw = localStorage.getItem(LS_KEY);
              if (raw) {
                const parsed = JSON.parse(raw);
                hasWizardData = !!(parsed?.memberId && parsed?.mafiaName);
              }
            } catch { }

            if (hasPendingClaim && hasWizardData) {
              // User completed wizard - show loading immediately and submit
              setS(p => ({ ...p, lookupLoading: false, submitting: true }));
              // Call submitAll directly to avoid flash of step 4
              submitAll();
            } else {
              // Fresh user without wizard data - clear memberId and show claim flow
              setS(p => ({ ...p, lookupLoading: false, memberId: "" }));
      ...
      if (isEdit && memberId) {
        // Set lookupLoading SYNCHRONOUSLY before clearing URL params
        // This prevents the localStorage effect from loading old state
        setS(p => ({ ...p, lookupLoading: true }));
        setInitializing(false);
  ...
  useEffect(() => {
    try {
      // Don't load if we're already in a loading state (e.g., edit flow already started)
      if (hasProcessedParams.current) {
        return;
      }

      // Always load localStorage - we need the wizard data (mafiaName, city, etc.)
      // even when coming back from OAuth with discordId in URL
      const raw = localStorage.getItem(LS_KEY);
      if (raw) {
        const parsed = JSON.parse(raw);
        ...
        setS((prev) => ({
          ...
          turtles: migratedTurtles,
          // Preserve submitting state - don't override if already submitting (e.g., returning from OAuth)
  ...
  async function claimRoles() {
    ...
    if (!s.discordId) {
      try {
        // Force persist wizard data BEFORE redirect to ensure it's available when we return
        // React's batched updates mean the persist effect may not have run yet
  ...
  // --- Render loading state during SSR, initialization, lookupLoading, or submitting ---
  if (!mounted || initializing || s.lookupLoading || s.submitting) {
    return (
      <div style={card()}>
        <div style={{ textAlign: "center", padding: "40px 0" }}>
          ...
          <p style={{ fontSize: 18, opacity: 0.8 }}>
            {s.submitting ? "Saving your profile..." : s.lookupLoading ? "Verifying member status..." : "Loading..."}
          </p>
          <style jsx>{`
